...
public class ShooterConfigFactory {
    ...
    public static SmartMotorControllerConfig createFlywheelMotorConfig(Subsystem subsystem) {
        ...
        // REV native velocity is RPM; YAMS' Spark wrapper currently expects rotations/sec.
        // Configure the encoder so both telemetry and closed-loop velocity use rotations/sec.
        vendorConfig.encoder.velocityConversionFactor(1.0 / 60.0);
        ...
        return new SmartMotorControllerConfig(subsystem)
            ...
            .withFeedforward(new SimpleMotorFeedforward(ShooterConstants.FLYWHEEL_KS, ShooterConstants.FLYWHEEL_KV)) // FF is applied in sim only via withSimFeedforward; real-robot uses PID-only for now.
